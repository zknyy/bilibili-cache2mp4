...
- `run-dir-all.py`
  ...
  - 生成前会预判目标文件，已存在且完好则跳过，详见「避免重复生成（断点续跑）」
...
每次转换前，脚本会先**预判**即将生成的完整路径与文件名，如果该文件已经存在
且校验通过，就跳过它，不再重复转换。因此中断后重新运行、或反复运行同一条
命令都是安全的：已经转好的不会被重做，只补做缺失的部分。

`run-dir-all.py` 判定「已有文件正常」时会依次检查：

1. 文件存在，且体积不小于 1 KB（排除空文件和半成品）
2. 顶层 box 恰好铺满整个文件（能识破被截断的文件——即使它的 `moov` 还完好、
   流信息和时长都读得出来）
3. 同时含视频流与音频流
4. 时长与 `videoInfo.json` 中的 `duration` 相符（允许 3 秒或 3% 的偏差，
   取较大者）
...
| 文件不存在 | 正常转换 |
| 已存在且校验通过 | **跳过**，不重复生成 |
| 已存在但校验不通过（损坏 / 被截断 / 缺流） | 删除旧文件，原地覆盖重做 |
| 已存在且完好，但时长与预期明显不符（同名却是另一个视频） | 保留原文件，另存为 `_1` 新名字 |

处理前打印的输出规划里会标出预判结果，例如
`[已存在且完好，跳过]`、`[已存在但不完好，将覆盖重做]`。

运行结束的汇总会分别列出新生成与跳过的数量：

```
处理完成：成功 2 个（新生成 1，跳过已存在 1），失败 0 个。
```
